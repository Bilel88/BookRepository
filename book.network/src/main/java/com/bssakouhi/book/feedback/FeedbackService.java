...
import com.bssakouhi.book.book.BookRepository;
...
public class FeedbackService {
    ...
    Book book =  bookRepository.findById(request.bookId())
            .orElseThrow(() -> new EntityNotFoundException("No book found with bthe ID::" + request.bookId()));
    if(book.isArchived() || !book.isShareable()){
        throw new OperationNotPermittedException("You cannot give a feedback for an archived or not shareable book");
    }
    User user = ((User) connectedUser.getPrincipal());
    if(Objects.equals(book.getOwner().getId(), user.getId())){
        throw new OperationNotPermittedException("You cannot give a feedback to your owner book");
    }
    ...
    public PageResponse<FeedbackResponse> findAllFeedbackByBook(Integer bookId, int page, int size, Authentication connectedUser) {
        ...
        List<FeedbackResponse> feedbacksResponses = feedbacks.stream()
                .map(f -> feedbackMapper.toFeedbackResponses(f, user.getId()))
                .toList();
